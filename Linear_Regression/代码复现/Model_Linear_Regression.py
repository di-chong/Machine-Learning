...
# 按比例得到训练和测试数据集
train_data = data.sample(frac = 0.8)      # sample：按比例随机选取若干行
...
# series.values将dataframe转化为ndarray
...
plt.scatter(x_train, y_train, label="Train data")
plt.scatter(x_test, y_test, label="Test data")
plt.plot(x_predications, y_predications, 'r', label="predict")
plt.xlabel(input_param_name)
plt.ylabel(output_param_name)
